...
@transform_df(
    ...
    df_stroke_subset=Input("ri.foundry.main.dataset.16af93e4-536c-4d4e-83b6-2df608128fa6"),
    stroke_post_cutoff=Input("ri.foundry.main.dataset.f72e9326-395c-40f3-a56d-6d073b6f53a3")
)
def compute(df_stroke_subset, stroke_post_cutoff):
    ...
    df_mapped_1 = utils_target.df_mapping(df_stroke_subset, "rural_urban_classification_null_removed", dict_name)
    ...
    columns_to_keep = utils_target.cols_to_keep.remove('first_cancer_diagnosis_at_death_after_cut_off')
    ...
    stroke_in_52_weeks = stroke_post_cutoff.filter(stroke_post_cutoff.stroke_diagnosis_in_next_52_weeks == 1)
    stroke_in_52_weeks = stroke_in_52_weeks.select(['patient_pseudo_id', 'stroke_diagnosis_in_next_52_weeks'])
