...
@transform_df(
    Output("/NHS/Cancer Late Presentation Likelihood Modelling Internal Transforms/cancer-late-datasets/interim-datasets/stroke_outcome_variable"),
...
def compute(df_comorbidities):
    ...
    specific_comorbidity_first = df_stroke_only.withColumn("row", F.row_number().over(w_asc)).filter(F.col("row") == 1).drop("row")
    specific_comorbidity_first = specific_comorbidity_first.withColumnRenamed("date", "stroke_after_cut_off_date_earliest")
    specific_comorbidity_first = specific_comorbidity_first.withColumnRenamed("diagnosis", "diagnosis_earliest_after_cut_off_date")
    ...
    specific_comorbidity_latest = df_stroke_only.withColumn("row", F.row_number().over(w_desc)).filter(F.col("row") == 1).drop("row")
    specific_comorbidity_latest = specific_comorbidity_latest.withColumnRenamed("date", "stroke_after_cut_off_date_latest")
    specific_comorbidity_latest = specific_comorbidity_latest.withColumnRenamed("diagnosis", "diagnosis_latest_after_cut_off_date")
    specific_comorbidity_latest = specific_comorbidity_latest.drop("source")

    # identify all of the unique cancer diagnoses
    df_unique_stroke_list = df_stroke_only.groupby("patient_pseudo_id").agg(F.collect_set("diagnosis").alias("all_stroke_diagnoses_after_cut_off"))

    # merge the tables

    df_stroke_after_cut_off = specific_comorbidity_first.join(specific_comorbidity_latest,
                                                              "patient_pseudo_id",
                                                              "left")

    df_stroke_after_cut_off = df_stroke_after_cut_off.join(df_unique_stroke_list,
                                                           "patient_pseudo_id",
                                                           "left")
    ...
    for weeks in target_weeks:
        ...
        df_stroke_after_cut_off = df_stroke_after_cut_off.withColumn("stroke_diagnosis_in_next_"+str(weeks)+"_weeks",
                                                                     F.when((F.col("stroke_after_cut_off_date_earliest") <= later_date), 1)
                                                                     .otherwise(0))
