...
def compute(patient_health_record):
    ...
    patient_health_record_latest = (
        patient_health_record.withColumn("row", F.row_number().over(w_desc))
        .filter(F.col("row") == 1)
        .drop("row")
    )
    ...
    for cat_variable in list_categorical_variables_from_phr:
        patient_health_record_latest = utils.clean_column_of_strings(
            patient_health_record_latest,
        ...
        expression_cols = utils.create_expressions_categorical_column(
            patient_health_record_latest, col_name=cat_variable + "_null_removed"
        )
    ...
    patient_health_record_latest = patient_health_record_latest.select(
        F.col("*"), *all_col_expressions
    )
