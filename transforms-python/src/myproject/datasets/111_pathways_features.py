...
def compute(source_df, cancer_symptoms):
    ...
    source_df = source_df.join(cancer_symptoms,
                               "symptom_group",
                               "left")
    ...
    source_df_cancer_calls_only = source_df_cancer_calls_only.withColumn("symptom_group_no_spaces",
                                                                         F.regexp_replace("symptom_group",
                                                                         r" |,|\(|\)",
                                                                         "_"))

    # total number of calls by ID
    dataset_111_calls = source_df.groupBy("patient_pseudo_id").count().withColumnRenamed("count", "111_calls_total")
    ...
    for weeks in weeks_to_subtract:
        ...
        total_number_calls_in_period = utils.number_calls_during_time_period(df=source_df,
                                                                             date=date_cutoff,
        ...
        total_number_cancer_calls_in_period = utils.number_calls_during_time_period(df=source_df_cancer_calls_only,
                                                                                    date=date_cutoff,
                                                                                    num_weeks_to_subtract=weeks,
        ...
        total_number_symptom_calls_in_period = utils.number_symptom_calls_during_time_period(df=source_df_cancer_calls_only,
                                                                                             date=date_cutoff,
                                                                                             num_weeks_to_subtract=weeks,
        ...
        dataset_111_calls = dataset_111_calls.join(total_number_calls_in_period,
                                                   "patient_pseudo_id",
                                                   "left")

        # join the total number of cancer related calls in the period
        dataset_111_calls = dataset_111_calls.join(total_number_cancer_calls_in_period,
                                                   "patient_pseudo_id",
                                                   "left")

        # join number of symptom calls in the period
        dataset_111_calls = dataset_111_calls.join(total_number_symptom_calls_in_period,
                                                   "patient_pseudo_id",
                                                   "left")
