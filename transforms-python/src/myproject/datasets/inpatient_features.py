...
def compute(inpatient_activity):
    ...
    inpatient_activity = inpatient_activity.filter(
        F.col("attendance_date") < F.lit(date_cutoff)
    )

    # total number of attendances by ID
    dataset_ip_attend = (
        inpatient_activity.groupBy("patient_pseudo_id")
        .count()
        .withColumnRenamed("count", "IP_attend_total")
    )
    ...
    for weeks in weeks_to_subtract:
        ...
        total_number_IP_in_period = utils.number_attendances_during_time_period(
            df=inpatient_activity,
            date=date_cutoff,
        ...
        dataset_ip_attend = dataset_ip_attend.join(
            total_number_IP_in_period, "patient_pseudo_id", "left"
        )

    # Determine number of diagnoses from IP
    df_inpatient_with_non_empty_diagnosis = inpatient_activity.filter(
        (F.col("dimention_8") != "") & (F.col("dimention_8").isNotNull())
    )

    dataset_IP_diagnoses = (
        df_inpatient_with_non_empty_diagnosis.groupBy("patient_pseudo_id")
        .count()
        .withColumnRenamed("count", "IP_diagnoses_total")
    )

    # Join table of count of attendances and diagnoses
    IP_features = dataset_ip_attend.join(
        dataset_IP_diagnoses, "patient_pseudo_id", "left"
    )
