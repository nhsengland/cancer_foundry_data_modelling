...
@transform_df(
    ...
    df_outpatient_with_diagnosis=Input(
        "ri.foundry.main.dataset.e3a5741d-6cb5-4b64-8d3b-0be26b1b6e62"
    ),
)
def compute(df_outpatient_with_diagnosis):
    ...
    # remove null IDs
    df_outpatient_with_diagnosis = df_outpatient_with_diagnosis.na.drop(
        subset=["patient_pseudo_id"]
    )

    # Filter to time before cut-off
    df_outpatient_with_diagnosis = df_outpatient_with_diagnosis.filter(
        F.col("attendance_date") < F.lit(date_cutoff)
    )

    # total number of attendances by ID
    dataset_opa_attend = (
        df_outpatient_with_diagnosis.groupBy("patient_pseudo_id")
        .count()
        .withColumnRenamed("count", "opa_attend_total")
    )
    ...
    for weeks in weeks_to_subtract:
        ...
        total_number_OPA_in_period = utils.number_attendances_during_time_period(
            df=df_outpatient_with_diagnosis,
            date=date_cutoff,
        ...
        dataset_opa_attend = dataset_opa_attend.join(
            total_number_OPA_in_period, "patient_pseudo_id", "left"
        )

    # Determine number of diagnoses from OPA
    df_outpatient_with_non_empty_diagnosis = df_outpatient_with_diagnosis.filter(
        (F.col("Primary_Diagnosis_Code") != "")
        & (F.col("Primary_Diagnosis_Code").isNotNull())
    )

    dataset_opa_diagnoses = (
        df_outpatient_with_non_empty_diagnosis.groupBy("patient_pseudo_id")
        .count()
        .withColumnRenamed("count", "opa_diagnoses_total")
    )

    # Join table of count of attendances and diagnoses
    opa_features = dataset_opa_attend.join(
        dataset_opa_diagnoses, "patient_pseudo_id", "left"
    )
    ...
    opa_features = opa_features.withColumn(
