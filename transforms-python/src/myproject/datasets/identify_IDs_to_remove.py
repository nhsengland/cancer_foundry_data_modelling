...
def compute(patient_features_expanded, cancer_diagnosis):
    ...
    ids_with_previous_or_active_cancer_phr = patient_features_expanded.filter(
        (F.col("phr_active_cancer") == 1) | (F.col("phr_cancer") == 1)
    ).select("patient_pseudo_id")
    ids_with_previous_or_active_cancer_phr = (
        ids_with_previous_or_active_cancer_phr.withColumn(
            "reason", F.lit("PHR flag of active or previous cancer")
        )
    )

    # remove IDs with previous cancer diagnosis based on OPA/IP/AEA/ECDS
    ids_with_previous_cancer_based_on_SUS_datasets = cancer_diagnosis.select(
        "patient_pseudo_id"
    )
    ids_with_previous_cancer_based_on_SUS_datasets = (
        ids_with_previous_cancer_based_on_SUS_datasets.withColumn(
            "reason", F.lit("Previous cancer flag from SUS")
        )
    )
    ...
    patient_features_expanded = utils.flag_if_patient_passed_away_with_cancer(
        patient_features_expanded
    )

    # remove IDs who passed away after the cut off date with a reason other than cancer
    ids_who_passed_away_after_cut_off_reason_other_than_cancer = (
        patient_features_expanded.filter(
            (F.col("death_month").isNotNull())
            & (F.col("death_month") >= date_cutoff)
            & (F.col("passed_away_with_cancer_cod") == 0)
        ).select("patient_pseudo_id")
    )
