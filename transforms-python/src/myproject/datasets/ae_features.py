...
@transform_df(
    ...
    df_emergency_activity=Input("ri.foundry.main.dataset.0cc8c784-f957-47b5-9844-a69d7eee0f6a"),
)
def compute(df_emergency_activity):
    ...
    df_emergency_activity = df_emergency_activity.filter(F.col("attendance_date") < F.lit(date_cutoff))

    # total number of emergency attendances by ID
    dataset_ae_attend = df_emergency_activity.groupBy("patient_pseudo_id").count().withColumnRenamed("count", "ae_attend_total")
    ...
    df_aea_with_non_empty_diagnosis = df_emergency_activity.filter((F.col("dimention_8") != '') & (F.col("dimention_8").isNotNull()))

    dataset_aea_diagnoses = df_aea_with_non_empty_diagnosis.groupBy("patient_pseudo_id").count().withColumnRenamed("count", "aea_diagnoses_total")

    # join AEA attendances with diagnoses

    dataset_ae_features = dataset_ae_attend.join(dataset_aea_diagnoses, "patient_pseudo_id", "left")

    # fill nulls with 0
    dataset_ae_features = dataset_ae_features.na.fill(0)

    # calculate percentage of A&E attendances that had a diagnosis
    dataset_ae_features = dataset_ae_features.withColumn("ae_diagnosis_pct_of_attendnaces",
