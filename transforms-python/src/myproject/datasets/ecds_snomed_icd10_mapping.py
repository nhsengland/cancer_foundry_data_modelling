...
@transform_df(
    Output(
        ecds_snomed_mapping_path
    ),
    df_ecds_etos=Input(ecds_etos_mapping_path),
    df_udal_map=Input(udal_icd10_mapping_path),
)
def compute(df_ecds_etos, df_udal_map):
    ...
    df_ecds_etos_non_deprecated = df_ecds_etos.filter(
        F.col("ICD10_Mapping").isNotNull()
    )
    ...
    w2 = Window.partitionBy("Referenced_Component_ID").orderBy(
        F.col("Map_Block").asc(), F.col("Map_Group").asc(), F.col("Map_Priority").desc()
    )

    df_udal_map_single_per_snomed = (
        df_udal_map.withColumn("row", F.row_number().over(w2))
        .filter(F.col("row") == 1)
        .drop("row")
    )

    # Join the udal map to the table where the ICD10 codes were null
    df_joined_deprecated = df_ecds_etos_deprecated.join(
        df_udal_map_single_per_snomed,
    ...
    df_joined_deprecated = df_joined_deprecated.withColumn(
        "ICD10_Mapping",
        F.when(F.col("ICD10_Mapping").isNull(), F.col("Map_Target")).otherwise(
            F.col("ICD10_Mapping")
        ),
    )
