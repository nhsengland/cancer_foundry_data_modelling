...
def compute(source_df):
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C00")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C32"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C15")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C26"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        "lung_after_cut_off",
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C33")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C45"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C40")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C49"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C43"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C47")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C72"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C50"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        "gynae_after_cut_off",
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C51")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C58"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C60")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C68"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C73")
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C74")
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C75"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C81")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C96"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C76")
            ...
            | F.col("all_cancer_diagnoses_after_cut_off").contains("C97"),
            1,
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C56"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C25"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C67"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C90"), 1
        ).otherwise(0),
    )
    ...
    df = df.withColumn(
        ...
        F.when(
            F.col("all_cancer_diagnoses_after_cut_off").contains("C16"), 1
        ).otherwise(0),
    )
