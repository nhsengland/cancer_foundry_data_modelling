...
@transform_df(
    Output("/NHS/Cancer Late Presentation Likelihood Modelling Internal Transforms/cancer-late-datasets/interim-datasets/outcome_variable"),
...
def compute(df_comorbidities):
    ...
    specific_comorbidity_first = df_cancer_only.withColumn("row", F.row_number().over(w_asc)).filter(F.col("row") == 1).drop("row")
    specific_comorbidity_first = specific_comorbidity_first.withColumnRenamed("date", "cancer_after_cut_off_date_earliest")
    ...
    specific_comorbidity_latest = df_cancer_only.withColumn("row", F.row_number().over(w_desc)).filter(F.col("row") == 1).drop("row")
    specific_comorbidity_latest = specific_comorbidity_latest.withColumnRenamed("date", "cancer_after_cut_off_date_latest")
    ...
    df_unique_cancer_list = df_cancer_only.groupby("patient_pseudo_id").agg(F.collect_set("diagnosis").alias("all_cancer_diagnoses_after_cut_off"))

    # merge the tables

    df_cancer_after_cut_off = specific_comorbidity_first.join(specific_comorbidity_latest,
                                                              "patient_pseudo_id",
                                                              "left")

    df_cancer_after_cut_off = df_cancer_after_cut_off.join(df_unique_cancer_list,
                                                           "patient_pseudo_id",
                                                           "left")
    ...
    for weeks in target_weeks:
        ...
        df_cancer_after_cut_off = df_cancer_after_cut_off.withColumn("cancer_diagnosis_in_next_"+str(weeks)+"_weeks",
                                                                     F.when((F.col("cancer_after_cut_off_date_earliest") <= later_date), 1)
                                                                     .otherwise(0))
