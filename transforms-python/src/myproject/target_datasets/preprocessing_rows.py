...
@transform_df(
    ...
    df_ids_to_remove=Input("ri.foundry.main.dataset.b31e0565-7803-4729-8805-961db7279f07"),
)
def compute(patient_features_expanded, df_ids_to_remove):
    ...
    df_patient_features_subset_patients = patient_features_expanded.join(df_ids_to_be_removed, "patient_pseudo_id", "left")
    ...
    df_patient_features_subset_patients = utils.flag_if_patient_passed_away_with_cancer(df_patient_features_subset_patients)

    # identifying those whose first diagnosis of cancer after the cut-off date is as a cause of death
    df_patient_features_subset_patients = df_patient_features_subset_patients.withColumn("first_cancer_diagnosis_at_death_after_cut_off",
                                                                                        F.when(
                                                                                            (F.col("passed_away_with_cancer_cod")==1) & 
                                                                                            (F.col("all_cancer_diagnoses_after_cut_off").isNull()) & 
                                                                                            (F.col("death_month")>date_cutoff),1).otherwise(0))
    ...
    for weeks in target_weeks:
        ...
        df_patient_features_subset_patients = df_patient_features_subset_patients.withColumn("death_with_first_cancer_diagnosis_in_next_"+str(weeks)+"_weeks",
                                                                                            F.when((F.col("death_month") <= later_date) & 
                                                                                            (F.col("first_cancer_diagnosis_at_death_after_cut_off")==1), 1)
                                                                                            .otherwise(0))
        ...
        df_patient_features_subset_patients = df_patient_features_subset_patients.withColumn("cancer_diagnosis_in_next_"+str(weeks)+"_weeks",
                                                                                            F.when((F.col("death_with_first_cancer_diagnosis_in_next_"+str(weeks)+"_weeks") == 1) | 
                                                                                            (F.col("cancer_diagnosis_in_next_"+str(weeks)+"_weeks")==1), 1)
                                                                                            .otherwise(0))
